...
function createAdminBootstrapScope(url, match2) {
  const sortedSearch = Array.from(url.searchParams.entries()).filter(([key]) => key !== ADMIN_BOOTSTRAP_PATH_PARAM).sort(([leftKey, leftValue], [rightKey, rightValue]) => {
    if (leftKey === rightKey) {
      return leftValue.localeCompare(rightValue);
    }
    return leftKey.localeCompare(rightKey);
...
function buildAdminBootstrapScopeHash(url, match2) {
  return hashObject(createAdminBootstrapScope(url, match2));
}
...
function resolveAdminBootstrapCachePolicy(url) {
  const range = url.searchParams.get("range") || void 0;
  const cacheType = ETagCacheManager.inferCacheType(normalizeAdminPagePath(url.pathname), range);
  return CACHE_CONFIGS[cacheType];
}
__name(resolveAdminBootstrapCachePolicy, "resolveAdminBootstrapCachePolicy");
...
async function serveAdminPageBootstrap(request, env, page, requestedHash) {
  ...
  if (requestedHash !== canonicalHash) {
    ...
    redirectUrl.searchParams.set(ADMIN_BOOTSTRAP_PATH_PARAM, url.pathname);
    return Response.redirect(redirectUrl.toString(), 307);
  }
  const workerVersion = getWorkerVersionInfo(env);
  const cachePolicy = resolveAdminBootstrapCachePolicy(url);
  ...
  const etag = ETagGenerator.generate(
    data,
    `page-bootstrap-${match2.page}-${workerVersion.namespace}-${canonicalHash}`
  );
  const headers = new Headers({
...
function buildDashboardBootstrapScopeHash(scope) {
  return hashObject2(normalizeDashboardScope(scope));
}
