...
var UnifiedCacheManager = class {
  ...
  async get(key, config = {}) {
    return this.fetch(new Request(this.createEdgeRequest(key)), async () => {
      throw new Error("Cache miss with get() helper");
    }, {
      strategy: "cache-only" /* CACHE_ONLY */,
      cacheKey: key,
      edgeTTL: config.edgeTTL ?? 300,
      workersTTL: config.workersTTL ?? 60
    });
  }
  async write(key, data, config) {
  ...
  async cacheFirst(key, fetcher, edgeTTL, workersTTL, strategy, providedEtag) {
  ...
  async networkFirst(key, fetcher, edgeTTL, workersTTL, strategy, providedEtag) {
    try {
  ...
  async staleWhileRevalidate(key, fetcher, edgeTTL, workersTTL, strategy, providedEtag) {
    const lookupStartedAt = this.now();
    const memoryResult = this.memoryCache.get(key);
    if (memoryResult) {
      sharedStats.workersHits++;
  ...
  async cacheOnly(key, edgeTTL, workersTTL, strategy) {
    const lookupStartedAt = this.now();
    const memoryResult = this.memoryCache.get(key);
    if (memoryResult) {
      return {
  ...
  async fetchAndCache(key, fetcher, edgeTTL, workersTTL, strategy, lookupMs, providedEtag) {
  ...
  createMeta(source, key, strategy, edgeTTL, workersTTL, values) {
    return {
      source,
      key,
      strategy,
...
var SSECacheNotificationService = class {
  ...
  async notifyBootstrapUpdatedMany(payloads, userId) {
...
function createDashboardPageScope(url) {
  ...
  return {
    page: "dashboard",
    range,
    from: useAbsoluteDates ? url.searchParams.get("from") || void 0 : void 0,
    to: useAbsoluteDates ? url.searchParams.get("to") || void 0 : void 0,
...
__name(normalizeMetricName, "normalizeMetricName");
...
function serializeServerTiming(metrics) {
  return metrics.map(serializeMetric).filter((metric) => Boolean(metric)).join(", ");
}
__name(serializeServerTiming, "serializeServerTiming");
function appendServerTiming(headers, metrics) {
...
__name(appendServerTiming, "appendServerTiming");
function nowMs() {
  return typeof performance !== "undefined" ? performance.now() : Date.now();
}
__name(nowMs, "nowMs");
function durationMs(startedAt, endedAt = nowMs()) {
  return normalizeDuration(endedAt - startedAt);
}
__name(durationMs, "durationMs");

// src/services/bootstrap/bootstrap-versioning.ts
function getBootstrapVersioningPolicy(cacheType) {
  const isRealtime = cacheType === "realtime";
  const currentEdgeTTL = isRealtime ? 60 : 300;
  const objectEdgeTTL = isRealtime ? 21600 : 86400;
  return {
    currentEdgeTTL,
...
__name(getBootstrapVersioningPolicy, "getBootstrapVersioningPolicy");
function buildBootstrapCurrentCacheKey(page, scopeHash, namespace) {
  return CacheKeyBuilder.custom(["bootstrap", page, scopeHash, "current", namespace]);
}
__name(buildBootstrapCurrentCacheKey, "buildBootstrapCurrentCacheKey");
function buildBootstrapObjectCacheKey(page, scopeHash, contentVersion, namespace) {
  return CacheKeyBuilder.custom(["bootstrap", page, scopeHash, "object", contentVersion, namespace]);
}
__name(buildBootstrapObjectCacheKey, "buildBootstrapObjectCacheKey");
function buildBootstrapContentVersion(data, page, workerVersion, scopeHash) {
  const etag = ETagGenerator.generate(data, `${page}-${workerVersion.namespace}-${scopeHash}`);
  return etag.replace(/^W\/"/, "").replace(/"$/, "");
}
__name(buildBootstrapContentVersion, "buildBootstrapContentVersion");
...
function createDashboardBootstrapQuery(scope) {
...
__name(createDashboardBootstrapQuery, "createDashboardBootstrapQuery");
...
function createDashboardBootstrapObjectQuery(scope, contentVersion) {
  const params = createDashboardBootstrapQuery(scope);
  params.set("__mode", "object");
  params.set("__version", contentVersion);
  return params;
}
__name(createDashboardBootstrapObjectQuery, "createDashboardBootstrapObjectQuery");
function buildDashboardBootstrapCacheKey(scope, namespace) {
  return buildBootstrapCurrentCacheKey("dashboard", buildDashboardBootstrapScopeHash(scope), namespace);
}
__name(buildDashboardBootstrapCacheKey, "buildDashboardBootstrapCacheKey");
function buildDashboardBootstrapObjectCacheKey(scope, contentVersion, namespace) {
  return buildBootstrapObjectCacheKey("dashboard", buildDashboardBootstrapScopeHash(scope), contentVersion, namespace);
}
__name(buildDashboardBootstrapObjectCacheKey, "buildDashboardBootstrapObjectCacheKey");
function createCurrentResponseHeaders(current, currentEtag, edgeTTL, swr) {
  return new Headers({
...
__name(createCurrentResponseHeaders, "createCurrentResponseHeaders");
function createObjectResponseHeaders(objectEtag, edgeTTL, swr, contentVersion) {
  return new Headers({
    "Content-Type": "application/json; charset=UTF-8",
    "Cache-Control": "public, max-age=0, must-revalidate",
    "CDN-Cache-Control": `public, s-maxage=${edgeTTL}, stale-while-revalidate=${swr}`,
...
__name(createObjectResponseHeaders, "createObjectResponseHeaders");
async function buildDashboardBootstrapArtifacts(env, scope, canonicalHash) {
  ...
  const staleAt = new Date(Date.parse(generatedAt) + versioningPolicy.currentEdgeTTL * 1e3).toISOString();
  const baseEnvelope = {
    page: "dashboard",
    ...bundle,
    generatedAt,
...
__name(buildDashboardBootstrapArtifacts, "buildDashboardBootstrapArtifacts");
async function serveDashboardBootstrap(request, env, requestedHash) {
  ...
  const { data: current, meta } = await cacheManager.fetch(
    request,
    async () => {
...
async function serveDashboardBootstrapObject(request, env, requestedHash, requestedVersion) {
  const requestStartedAt = nowMs();
  const url = new URL(request.url);
  const scope = createDashboardPageScope(url);
  const canonicalHash = buildDashboardBootstrapScopeHash(scope);
  ...
  try {
    const { data, etag, meta } = await cacheManager.get(objectKey, {
      edgeTTL: versioningPolicy.objectEdgeTTL,
      workersTTL: versioningPolicy.objectWorkersTTL
    });
    const objectEtag = etag || ETagGenerator.generate(
      data,
...
__name(serveDashboardBootstrapObject, "serveDashboardBootstrapObject");
...
function normalizeAdminPagePath(pathname) {
  if (pathname === "/l") {
    return "/landings";
  }
  return pathname;
}
__name(normalizeAdminPagePath, "normalizeAdminPagePath");
...
// src/services/bootstrap/admin-bootstrap.ts
var ADMIN_BOOTSTRAP_PATH_PARAM = "__pathname";
var TRANSIENT_BOOTSTRAP_QUERY_PARAMS = /* @__PURE__ */ new Set([
...
__name(toDateOnly2, "toDateOnly");
function getRecentDateRange2(days) {
  const end = /* @__PURE__ */ new Date();
  end.setHours(0, 0, 0, 0);
  const start = new Date(end);
  start.setDate(start.getDate() - days + 1);
  return {
    startDate: toDateOnly2(start),
    endDate: toDateOnly2(end)
  };
}
__name(getRecentDateRange2, "getRecentDateRange");
function isDateOnly(value) {
  return Boolean(value && /^\d{4}-\d{2}-\d{2}$/.test(value));
}
__name(isDateOnly, "isDateOnly");
function dateWindowIncludesToday(startDate, endDate) {
  const today = toDateOnly2(/* @__PURE__ */ new Date());
  return startDate <= today && today <= endDate;
}
__name(dateWindowIncludesToday, "dateWindowIncludesToday");
function resolveImplicitAdminDateWindow(url, match2) {
  const defaults = getRecentDateRange2(7);
  switch (match2.page) {
    case "campaign-detail":
    case "trends":
    case "audit":
...
__name(resolveImplicitAdminDateWindow, "resolveImplicitAdminDateWindow");
function normalizeAdminSearch(url, match2) {
  const defaults = getRecentDateRange2(7);
  const normalized = Array.from(url.searchParams.entries()).filter(([key]) => !TRANSIENT_BOOTSTRAP_QUERY_PARAMS.has(key)).sort(([leftKey, leftValue], [rightKey, rightValue]) => {
    if (leftKey === rightKey) {
      return leftValue.localeCompare(rightValue);
    }
    return leftKey.localeCompare(rightKey);
...
__name(normalizeAdminSearch, "normalizeAdminSearch");
function hashObject2(value) {
  const serialized = JSON.stringify(value);
  let hash = 0;
  for (let index = 0; index < serialized.length; index++) {
    hash = (hash << 5) - hash + serialized.charCodeAt(index) | 0;
...
__name(hashObject2, "hashObject");
function createAdminBootstrapScope(url, match2) {
  return {
    page: match2.page,
    pathname: normalizeAdminPagePath(url.pathname),
    params: match2.params,
    search: normalizeAdminSearch(url, match2)
  };
}
__name(createAdminBootstrapScope, "createAdminBootstrapScope");
function buildAdminBootstrapScopeHash(url, match2) {
  return hashObject2(createAdminBootstrapScope(url, match2));
}
__name(buildAdminBootstrapScopeHash, "buildAdminBootstrapScopeHash");
function buildAdminBootstrapPath(url, match2) {
  return `/__bootstrap/${match2.page}/${buildAdminBootstrapScopeHash(url, match2)}.json`;
}
__name(buildAdminBootstrapPath, "buildAdminBootstrapPath");
function createAdminBootstrapObjectQuery(url, match2, contentVersion) {
  const params = createAdminBootstrapQuery(url, match2);
  params.set("__mode", "object");
  params.set("__version", contentVersion);
  return params;
}
__name(createAdminBootstrapObjectQuery, "createAdminBootstrapObjectQuery");
function createAdminBootstrapQuery(url, match2) {
  const params = new URLSearchParams();
  for (const [key, value] of Object.entries(normalizeAdminSearch(url, match2))) {
    params.set(key, value);
  }
  params.set(ADMIN_BOOTSTRAP_PATH_PARAM, normalizeAdminPagePath(url.pathname));
  return params;
}
__name(createAdminBootstrapQuery, "createAdminBootstrapQuery");
function buildAdminBootstrapCacheKey(url, match2, namespace) {
  return buildBootstrapCurrentCacheKey(match2.page, buildAdminBootstrapScopeHash(url, match2), namespace);
}
__name(buildAdminBootstrapCacheKey, "buildAdminBootstrapCacheKey");
function buildAdminBootstrapObjectCacheKey(url, match2, contentVersion, namespace) {
  return buildBootstrapObjectCacheKey(match2.page, buildAdminBootstrapScopeHash(url, match2), contentVersion, namespace);
}
__name(buildAdminBootstrapObjectCacheKey, "buildAdminBootstrapObjectCacheKey");
function resolveAdminBootstrapRange(url, match2) {
  const rawRange = url.searchParams.get("range");
  if (rawRange) {
    return rawRange;
  }
  if (match2.page === "campaigns") {
    return "today";
  }
  const dateWindow = resolveImplicitAdminDateWindow(url, match2);
...
__name(resolveAdminBootstrapRange, "resolveAdminBootstrapRange");
function resolveAdminBootstrapPolicy(url, match2) {
  const cacheType = ETagCacheManager.inferCacheType(
    normalizeAdminPagePath(url.pathname),
    resolveAdminBootstrapRange(url, match2)
  );
  return getBootstrapVersioningPolicy(cacheType);
}
__name(resolveAdminBootstrapPolicy, "resolveAdminBootstrapPolicy");
function resolveSourceUrl(requestUrl, page) {
...
function createCurrentResponseHeaders2(current, currentEtag, edgeTTL, swr) {
  return new Headers({
    "Content-Type": "application/json; charset=UTF-8",
    "Cache-Control": "public, max-age=0, must-revalidate",
    "CDN-Cache-Control": `public, s-maxage=${edgeTTL}, stale-while-revalidate=${swr}`,
...
__name(createCurrentResponseHeaders2, "createCurrentResponseHeaders");
function createObjectResponseHeaders2(objectEtag, edgeTTL, swr, contentVersion) {
  return new Headers({
    "Content-Type": "application/json; charset=UTF-8",
    "Cache-Control": "public, max-age=0, must-revalidate",
    "CDN-Cache-Control": `public, s-maxage=${edgeTTL}, stale-while-revalidate=${swr}`,
...
__name(createObjectResponseHeaders2, "createObjectResponseHeaders");
async function buildAdminBootstrapArtifacts(env, url, match2, canonicalHash) {
  const versioningPolicy = resolveAdminBootstrapPolicy(url, match2);
  const workerVersion = getWorkerVersionInfo(env);
  const bundle = await buildAdminPageBundle(env, url, match2);
  const generatedAt = bundle.generatedAt || (/* @__PURE__ */ new Date()).toISOString();
  const staleAt = new Date(Date.parse(generatedAt) + versioningPolicy.currentEdgeTTL * 1e3).toISOString();
  const baseEnvelope = {
    ...bundle,
    scopeHash: canonicalHash,
  ...
  const contentVersion = buildBootstrapContentVersion(baseEnvelope, `${match2.page}-bootstrap`, workerVersion, canonicalHash);
  const envelope = {
    ...baseEnvelope,
    contentVersion
  };
  const objectEtag = ETagGenerator.generate(
    envelope,
    `page-bootstrap-object-${match2.page}-${workerVersion.namespace}-${canonicalHash}-${contentVersion}`
  );
  const objectUrl = `${buildAdminBootstrapPath(url, match2)}?${createAdminBootstrapObjectQuery(url, match2, contentVersion).toString()}`;
  const current = {
    page: match2.page,
    scopeHash: canonicalHash,
    scope: envelope.scope,
  ...
  return {
    ...
    current,
    objectEtag,
    currentKey: buildAdminBootstrapCacheKey(url, match2, workerVersion.namespace),
    objectKey: buildAdminBootstrapObjectCacheKey(url, match2, contentVersion, workerVersion.namespace)
  };
}
__name(buildAdminBootstrapArtifacts, "buildAdminBootstrapArtifacts");
async function serveAdminPageBootstrap(request, env, page, requestedHash) {
  const requestStartedAt = nowMs();
  const requestUrl = new URL(request.url);
  const url = resolveSourceUrl(requestUrl, page);
  const match2 = url ? matchAdminPage(url) : null;
  if (!url || !match2 || match2.page !== page) {
    return new Response("Not Found", { status: 404 });
  }
  const canonicalHash = buildAdminBootstrapScopeHash(url, match2);
  if (requestedHash !== canonicalHash) {
    const redirectUrl = new URL(request.url);
    redirectUrl.pathname = buildAdminBootstrapPath(url, match2);
    redirectUrl.search = createAdminBootstrapQuery(url, match2).toString();
    return Response.redirect(redirectUrl.toString(), 307);
  }
  const versioningPolicy = resolveAdminBootstrapPolicy(url, match2);
  const cacheManager = new UnifiedCacheManager(env);
  const workerVersion = getWorkerVersionInfo(env);
  const currentKey = buildAdminBootstrapCacheKey(url, match2, workerVersion.namespace);
  const { data: current, meta } = await cacheManager.fetch(
    request,
    async () => {
      const artifacts = await buildAdminBootstrapArtifacts(env, url, match2, canonicalHash);
      await cacheManager.write(artifacts.objectKey, artifacts.envelope, {
        edgeTTL: artifacts.versioningPolicy.objectEdgeTTL,
        workersTTL: artifacts.versioningPolicy.objectWorkersTTL,
        etag: artifacts.objectEtag
  ...
  const currentEtag = ETagGenerator.generate(
    current,
    `page-bootstrap-current-${match2.page}-${workerVersion.namespace}-${canonicalHash}`
  );
  const headers = createCurrentResponseHeaders2(
    current,
    currentEtag,
    versioningPolicy.currentEdgeTTL,
    versioningPolicy.currentSWR
  ...
  const metrics = [
    { name: "cache", dur: meta.totalMs, desc: meta.source },
    { name: "lookup", dur: meta.lookupMs, desc: meta.strategy },
    { name: "build", dur: meta.originMs, desc: meta.source === "origin" ? `${match2.page}+pointer` : "cached" },
    { name: "cachewrite", dur: meta.writeMs, desc: meta.source === "origin" ? "refresh" : "skip" },
    { name: "etag", desc: currentEtag },
    { name: "ttl", desc: `edge=${versioningPolicy.currentEdgeTTL};swr=${versioningPolicy.currentSWR}` }
  ];
  if (ETagGenerator.matches(request.headers.get("If-None-Match"), currentEtag)) {
    appendServerTiming(headers, [
      ...metrics,
      { name: "reval", desc: "304" },
      { name: "total", dur: durationMs(requestStartedAt), desc: `${match2.page}-bootstrap-current` }
    ]);
    return new Response(null, { status: 304, headers });
  }
  appendServerTiming(headers, [
...
__name(serveAdminPageBootstrap, "serveAdminPageBootstrap");
async function serveAdminPageBootstrapObject(request, env, page, requestedHash, requestedVersion) {
  const requestStartedAt = nowMs();
  const requestUrl = new URL(request.url);
  const url = resolveSourceUrl(requestUrl, page);
  const match2 = url ? matchAdminPage(url) : null;
  ...
  if (requestedHash !== canonicalHash) {
    const redirectUrl = new URL(request.url);
    redirectUrl.pathname = buildAdminBootstrapPath(url, match2);
    redirectUrl.search = createAdminBootstrapObjectQuery(url, match2, requestedVersion).toString();
    return Response.redirect(redirectUrl.toString(), 307);
  }
  const versioningPolicy = resolveAdminBootstrapPolicy(url, match2);
  const cacheManager = new UnifiedCacheManager(env);
  const workerVersion = getWorkerVersionInfo(env);
  const objectKey = buildAdminBootstrapObjectCacheKey(url, match2, requestedVersion, workerVersion.namespace);
  try {
    const { data, etag, meta } = await cacheManager.get(objectKey, {
      edgeTTL: versioningPolicy.objectEdgeTTL,
      workersTTL: versioningPolicy.objectWorkersTTL
    ...
    const objectEtag = etag || ETagGenerator.generate(
      data,
      `page-bootstrap-object-${match2.page}-${workerVersion.namespace}-${canonicalHash}-${requestedVersion}`
    );
    const headers = createObjectResponseHeaders2(
      objectEtag,
      versioningPolicy.objectEdgeTTL,
      versioningPolicy.objectSWR,
      requestedVersion
    ...
    if (ETagGenerator.matches(request.headers.get("If-None-Match"), objectEtag)) {
      appendServerTiming(headers, [
        ...metrics,
        { name: "reval", desc: "304" },
        { name: "total", dur: durationMs(requestStartedAt), desc: `${match2.page}-bootstrap-object` }
      ]);
      return new Response(null, { status: 304, headers });
    }
    appendServerTiming(headers, [
    ...
    if (artifacts.envelope.contentVersion !== requestedVersion) {
      const redirectUrl = new URL(request.url);
      redirectUrl.pathname = buildAdminBootstrapPath(url, match2);
      redirectUrl.search = createAdminBootstrapObjectQuery(url, match2, artifacts.envelope.contentVersion).toString();
      return Response.redirect(redirectUrl.toString(), 307);
    }
    const writeStartedAt = nowMs();
    await cacheManager.write(artifacts.objectKey, artifacts.envelope, {
    ...
    const metrics = [
      { name: "cache", dur: originMs + writeMs, desc: "origin" },
      { name: "lookup", dur: 0, desc: "cache-only" },
      { name: "build", dur: originMs, desc: `${match2.page}+object` },
      { name: "cachewrite", dur: writeMs, desc: "refresh" },
      { name: "etag", desc: artifacts.objectEtag },
...
__name(serveAdminPageBootstrapObject, "serveAdminPageBootstrapObject");
...
function toDateOnly3(value) {
  return value.toISOString().split("T")[0];
}
__name(toDateOnly3, "toDateOnly");
function getRecentDateRange3(days) {
  const end = /* @__PURE__ */ new Date();
  end.setHours(0, 0, 0, 0);
  const start = new Date(end);
  start.setDate(start.getDate() - days + 1);
  return {
    startDate: toDateOnly3(start),
    endDate: toDateOnly3(end)
  };
}
__name(getRecentDateRange3, "getRecentDateRange");
var CacheUpdateService = class {
  ...
  async warmCache() {
    ...
    try {
      ...
      errors.push(`Admin bootstraps: ${error2 instanceof Error ? error2.message : "Unknown error"}`);
    }
    return {
  ...
  getDashboardBootstrapKeys(ranges) {
  ...
  async invalidateDashboardBootstrap(range) {
    await this.cacheManager.invalidateBatch(this.getDashboardBootstrapKeys([range]));
  }
  async warmupDashboardBootstrap(range) {
    const url = new URL(`https://cache.local/dashboard?range=${encodeURIComponent(range)}`);
    const scope = createDashboardPageScope(url);
  ...
  getAdminWarmPagesForEntity(entity) {
    switch (entity) {
      case "campaign":
      case "campaigns":
  ...
  getAdminBootstrapWarmSpecs(pages) {
    const defaults = getRecentDateRange3(7);
    const specs = [
  ...
  getRealtimeAdminBootstrapWarmSpecs() {
    const defaults = getRecentDateRange3(7);
    return [
  ...
  getAdminBootstrapKeysForPages(specs) {
    const namespace = getWorkerVersionInfo(this.env).namespace;
    return specs.flatMap((spec) => {
      const match2 = matchAdminPage(spec.url);
      if (!match2) {
        return [];
      }
      return [buildAdminBootstrapCacheKey(spec.url, match2, namespace)];
    });
  }
  getAdminBootstrapKeysForEntity(entity) {
    return this.getAdminBootstrapKeysForPages(this.getAdminBootstrapWarmSpecs(this.getAdminWarmPagesForEntity(entity)));
  }
  async invalidateAdminBootstrapPages(pages) {
  ...
  async invalidateAdminBootstrapSpecs(specs) {
  ...
  async warmupAdminBootstrapSpecs(specs) {
    ...
    for (const spec of specs) {
      ...
      const requestedHash = buildAdminBootstrapPath(spec.url, match2).split("/").pop()?.replace(/\.json$/i, "") || "";
      const requestUrl = new URL(`https://cache.local/__bootstrap/${match2.page}/${requestedHash}.json`);
      for (const [key, value] of spec.url.searchParams.entries()) {
        requestUrl.searchParams.set(key, value);
      }
      requestUrl.searchParams.set("__pathname", spec.url.pathname);
      const response = await serveAdminPageBootstrap(new Request(requestUrl.toString()), this.env, match2.page, requestedHash);
      const current = await response.json();
      updates.push({
        cacheKey: buildAdminBootstrapCacheKey(spec.url, match2, getWorkerVersionInfo(this.env).namespace),
        page: current.page,
        scopeHash: current.scopeHash,
        version: current.contentVersion
      });
...
function toStrongETag(weakEtag) {
  return `"${weakEtag.replace(/^W\/"/, "").replace(/"$/, "")}"`;
}
__name(toStrongETag, "toStrongETag");
var PRECOMPRESSED_RESPONSE_TYPES = /* @__PURE__ */ new Map([
...
var COMPRESSION_DEMO_HTML = `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
function getPreferredAssetEncoding(acceptEncodingHeader) {
...
__name(getPreferredAssetEncoding, "getPreferredAssetEncoding");
function getPrecompressedAssetContentType(pathname) {
...
__name(getPrecompressedAssetContentType, "getPrecompressedAssetContentType");
function isPrecompressedAssetRequest(request, pathname) {
...
__name(isPrecompressedAssetRequest, "isPrecompressedAssetRequest");
async function servePrecompressedAsset(request, env) {
  const url = new URL(request.url);
...
__name(servePrecompressedAsset, "servePrecompressedAsset");
async function servePrecompressedStaticAsset(request, env, assetPath, contentType) {
...
__name(servePrecompressedStaticAsset, "servePrecompressedStaticAsset");
async function serveCompressionDemo(request, env) {
  const url = new URL(request.url);
...
__name(serveCompressionDemo, "serveCompressionDemo");
app.use("*", logger());
app.use(
  "*",
  cors({
...
function isAppControlRequest(pathname) {
  return pathname === "/health" || pathname === "/sw.js" || pathname.startsWith("/__bootstrap/") || pathname.startsWith("/__compression-demo/") || pathname.startsWith("/api/") || pathname.startsWith("/events/");
}
__name(isAppControlRequest, "isAppControlRequest");
function resolveRequestedRange(pathname, url) {
...
__name(resolveRequestedRange, "resolveRequestedRange");
async function serveSpaShellHtml(request, env) {
  const requestStartedAt = nowMs();
  const url = new URL(request.url);
...
__name(serveSpaShellHtml, "serveSpaShellHtml");
var index_default = {
